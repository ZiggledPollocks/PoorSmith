...
public class CoalInteractable : MonoBehaviour, IInteractable, IResourceProvider
{
    [Header("Resource")]
    ...
    [SerializeField, Range(0f, 1f)] private float oneTierGapDropChance = 0.5f;
    [SerializeField, Range(0f, 1f)] private float twoOrMoreTierGapDropChance = 0.2f;
    ...
    [SerializeField, Min(1)] private int maxInteractCount = 6;
    ...
    public void Interact(PlayerInteraction interactionContext)
    {
        ...
        {
            Debug.LogWarning("석탄 ResourceData 또는 ItemDropSpawner가 연결되지 않았습니다.");
            return;
        }

        interactCount++;
        Debug.Log($"석탄 상호작용 횟수: {interactCount}");
    ...
    private bool ShouldDropResource(ToolData toolData)
    {
        ...
        Debug.Log(
            $"석탄 티어 {Tier}, 도구 티어 {toolData.Tier}: " +
