...
public class Storm_StoneInteractable : MonoBehaviour, IInteractable, IResourceProvider
{
    [Header("Resource")]
    ...
    public void Interact(PlayerInteraction interactionContext)
    {
        ...
        {
            Debug.LogWarning("돌풍석 ResourceData 또는 ItemDropSpawner가 연결되지 않았습니다.");
            return;
        }

        interactCount++;
        Debug.Log($"돌풍석 상호작용 횟수: {interactCount}");
    ...
    private bool ShouldDropResource(ToolData toolData)
    {
        ...
        Debug.Log(
            $"돌풍석 티어 {Tier}, 도구 티어 {toolData.Tier}: " +
